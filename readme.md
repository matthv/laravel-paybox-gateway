...
1. Run
   ...
   ```php   
   composer require matthv/laravel-paybox-gateway
   ``` 
...
2. Open `config/app.php` and add
   ...
   ```php
   Matthv\PayboxGateway\Providers\PayboxServiceProvider::class,
   ```
...
3. Run
    ...
    ```php
    php artisan vendor:publish --provider="Matthv\PayboxGateway\Providers\PayboxServiceProvider"
    ```
...
```php
$authorizationRequest = \App::make(\Matthv\PayboxGateway\Requests\AuthorizationWithCapture::class);
...
If you want more customization take a look at public methods of  `\Matthv\PayboxGateway\Requests\Authorization` class.

For `setAmount` default currency is Euro. If you want to use other currency, you should use currency constant from `\Matthv\PayboxGateway\Currency` class as 2nd parameter. Also please notice that amount you should give to this function is real amount (with decimal places) and not converted already to Paybox format.  
...
In case you use `AuthorizationWithoutCapture` you should make sure, you have `\Matthv\PayboxGateway\ResponseField::PAYBOX_CALL_NUMBER` and `\Matthv\PayboxGateway\ResponseField::TRANSACTION_NUMBER` in your return fields because those values will be needed when capturing payment later.  You should also always have `\Matthv\PayboxGateway\ResponseField::AUTHORIZATION_NUMBER` and `\Matthv\PayboxGateway\ResponseField::SIGNATURE` in your return fields and signature should be always last parameter. 
...
To make sure the payment was really successful you should use `\Matthv\PayboxGateway\Responses\Verify` class. The simplest code could look like this:

```php
$payment = Payment::where('number', $request->input('order_number'))->firstOrFail();
$payboxVerify = \App::make(\Matthv\PayboxGateway\Responses\Verify::class);
try {
...
$captureRequest = \App::make(\Matthv\PayboxGateway\Requests\Capture::class);
